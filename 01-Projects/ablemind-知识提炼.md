...
- [ ] U1 LangGraph + deepagents 图构建 ← **建议第一刀**
...
1. 从 U1 开始：`/auto-wiki ingest`（先 recall 读 `wiki/_index.md` + `wiki/agent/_ontology.md` 路由）。
2. U1 会一次性建出 LangGraph/deepagents 中心节点 + FastAPI/PostgreSQL/Docker 三个跨域 `built_on` 靶子，验证整条跨域机器。
3. 之后按 Wave 顺序推进，每个 ingest 后回来勾进度。

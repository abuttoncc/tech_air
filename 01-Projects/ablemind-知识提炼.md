...
- [x] U3 Middleware 分层与编排 ✅ 2026-06-17（agent: 中间件分层编排 + LangChain；回填 deepagents→implements、LangGraph→built_on LangChain。纯 agent 域）
- [x] U4 llm_router + thinking round-trip ✅ 2026-06-18（agent: 供应商中立路由 + 推理内容回传，均 built_on LangChain。Wave 1 封顶。provider 实体节点暂不建）
...
- **Wave 1 完成（U1–U4）**：agent 心脏齐了 — 框架栈 LangChain→LangGraph→deepagents + 四个模式/原理（状态化执行 / 协议桥接管道 / 中间件分层编排 / 供应商中立路由 / 推理内容回传）。图谱：agent 9 节点 / web 3 / data 2，跨域 built_on 已织 agent↔web↔data。
- **转 Wave 2 — 桥到 data**：建议 **U5 ExecutionContext 四层隔离 + 多租户**（复用 PostgreSQL，引入多租户隔离原理），再 U6/U7 把 data 域织厚（checkpoint 记忆 / message_events 事件溯源）。
- 之后 Wave 3 转 infra（U9 沙箱首次落 infra 域）。

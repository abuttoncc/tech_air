...
- **U1 已完成**（2026-06-17）：agent 域立住 LangGraph/deepagents/状态化执行，data 域立住 PostgreSQL，跨域 `built_on` 机器已验证（LangGraph→PostgreSQL）。
- **下一刀建议 U2（Protocol Bridge Pipeline）**：紧挨 U1 的心脏，会把 web 域第一个节点（AI SDK v6 SSE）经 `runs_on` 拉进来，并复用本来源页。
- 之后按 Wave 顺序推进，每个 ingest 后回来勾进度。
- 经验记录：ingest 时严格按「边准确性」决定建不建跨域靶子，宁可延后也不建越界边（FastAPI/Docker 即按此延后）。

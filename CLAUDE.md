...
1. **结构问题**（有哪些节点 / 怎么连 / 有无跨域边 / 邻居）→ 一发 `expand.py <seed> --json`（带 `cross` 跨域标记、边型、方向）或直接查 `data.db`（关系就是一条 SQL 的图）。**跨域判定库已编译好，别手 grep 再算。**
2. **内容问题**（定义准不准、正文细节）→ 用第 1 步的图谱**定位**，只 `cat` 存疑的少数页，不要无差别全读。

这是把「决策不旁路本体」用在 agent 自己的检索上：库的访问层比手抓 Bash 又快又准，因为答案早编译进去了。`recall`/`query` 模式就是这条路的封装，优先用。

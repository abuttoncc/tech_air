...
> `wiki/web/` 的本体定义。`ingest`/`recall` 前先读本页。引擎规范见 `.claude/skills/auto-wiki/`，
> 与 [[wiki/agent/_ontology|agent 母本]] **共用同一套 6 档时间模型、退役不删除协议与三分铁律**——
> **本页只声明 全栈应用 领域特有的节点类型与受控关系词表**，其余一律以引擎 + agent 母本 契约为准。
...
## 3. 受控关系词表（全栈应用 特有；通用词 references/part_of/instance_of 等沿用 agent 母本）
<在此声明本域的关系词：from_type → to_type，一句话语义。lint 时以此对照，越界边拒绝。>

## 4. 跨域连接
本域作为驱动层时，用 `references` 边连到其他域的「分析/节点」（如 web/分析 → equity/公司）。
